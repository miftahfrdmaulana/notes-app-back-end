...
const addNoteHandler = (request, h) => {
    ...
    // Memberikan respons sesuai isSuccess:
    if(isSuccess){
...
const editNoteByIdHandler = (request, h) => {
    ...
    // Jika index berisi maka niainya = id notes. jika tidak berisi maka nilainya -1:
    if(index !== -1){
        notes[index] = {
            ...notes[index],
            title,
...
const deleteNotesByIdHandler = (request, h) => {
    ...
    const response = h.response({
        status: 'fail',
        message: 'Gagal menghabus note: Id tidak ditemukan',
    });
    response.code(404);
    return response;
}
